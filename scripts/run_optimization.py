...
DEFAULT_CONFIG = Path("config/optimization_npt30_low_inc.json")
...
def _load_design_variables(payload: dict[str, Any]) -> tuple[ConfigDesignVariable, ...]:
    ...
    if not raw_variables:
        raise ValueError(
            ...
            "run scripts/run_design_point.py or add outer design variables before using run_optimization.py."
        )
