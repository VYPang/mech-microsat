...
from source.orbit.surrogate import ResponseSurface
...
def _load_surrogate(source: ResponseSurface | Path) -> ResponseSurface:
    ...
    if not path.exists():
        raise FileNotFoundError(
            "SRP surrogate JSON not found at "
            f"'{path}'. Run `uv run python scripts/run_sweep.py --config config/sweep_low_inc.json` "
            "or update orbit.surrogate_json_path to a generated surrogate before running the sizing solver."
        )
    return ResponseSurface.from_json(path)
