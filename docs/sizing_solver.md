...
- A project-local nonlinear solver in `source/optimization/analysis.py` through the `FixedPointAnalysis` class.
- SciPy only for the optional outer optimization script in `scripts/run_optimization.py` via `scipy.optimize.minimize`.
...
- `scripts/run_design_point.py`: the default sizing solver.
- `scripts/run_optimization.py`: optional outer optimization wrapper.
...
The full registry lives in `source/optimization/variables.py`.
...
```bash
python scripts/run_design_point.py \
    --config config/optimization_npt30_low_inc.json
```
...
The current wrapper for this is `scripts/run_optimization.py`, which uses `scipy.optimize.minimize`.
